...
object Drive_r {
  ...
  */
  ...
  def listFiles(service: Drive, fileId: String): List[(File, String)] = {
    ...
    list = (file, getParents(service, file, file.getName)) :: list // returns the given file/folder's path
    if (file.getMimeType == "application/vnd.google-apps.folder") { // returns a folder's children's paths
